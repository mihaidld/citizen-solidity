...
contract State {
    ...
    struct Citizen {
        ...
        uint256 delayAdmin; // till when is the citizen with admin status an admin follwing his election (8 weeks)
        ...
        uint256 delayBanned; // till when is the member banned following his punishment : block.timestamp + 10 * 52 * 1 weeks
    ...
    */
    ...
    struct Election {
        ...
        uint256 start; // when voting is possible (current mandate term - 1 week)
        uint256 term; // till when voting is possible (start + 1 week )
        ...
        mapping(address => uint256) nbOfVotes; // mapping to associate for each citizen the number of votes received
    }

    /// @dev struct Mandate after admin elections
    struct Mandate {
        uint256 id; // id of mandate
        uint256 start; // when mandate starts after election results
        uint256 term; // till when mandate (start + 8 weeks )
    }
    ...
    mapping(address => uint256) public companies; //when registered companies[address] = 5000 full tokens
    ...
    uint256 private counterIdElection;

    /// @dev counter for mandates id incremented by each mandate
    uint256 private counterIdMandate;
    ...
    //Other variables
    /// @dev punishment options to be voted by admins: 0 -> Blank, 1 -> Yes, 2 -> No, other -> Invalid vote
    string
        public sentencesOptions = "0 -> Small, 1 -> Moderate, 2 -> Serious, 3 -> Treason";
    ...
    uint256 private awardRegistration = 5000 * denomination;
    ...
    constructor(address _citizenAddress) public {
        token = CitizenERC20(_citizenAddress); //contract CitizenERC20 deployed at
    ...
    modifier onlyAdmin() {
        require(
            citizens[msg.sender].isAdmin == true,
    ...
    modifier onlyActiveCitizens() {
        require(
            ...
            "only citizens with at least 1 unit of CTZ can perform this action"
        );
        _;
    }
    ...
    modifier onlyAllowedCitizens() {
        require(
            citizens[msg.sender].delayBanned < block.timestamp,
        ...
        onlyAllowedCitizens
    {
        ...
        require(
            block.timestamp >= mandates[counterIdMandate].term - 1 weeks,
            "too early to elect an admin"
        );
        require(
        ...
        require(
            citizens[_candidateAddress].delayBanned < block.timestamp,
            "candidate is banned"
        );
        require(
            elections[counterIdElection].didVote[msg.sender] == false,
            "citizen already voted for this proposal"
        );
    ...
    function punish(address _sentenced, Punishment _option)
        public
        onlyAdmin
        onlyAliveCitizens
        onlyActiveCitizens
        onlyAllowedCitizens
    {
        ...
        if (_option == Punishment.Small) {
            ...
            citizens[_sentenced].delayBanned =
                block.timestamp +
                10 *
                52 *
                1 weeks;
    ...
    function pardon(address _beneficiary)
        public
        onlyAdmin
        onlyAliveCitizens
        onlyActiveCitizens
    ...
    function becomeCitizen(uint256 _age) public {
    ...
    /*     function registerCompany() public {
        ...
        require(
            companies[msg.sender].id == 0,
            "only for companies not yet registered"
        );
        counterIdCompany++;
